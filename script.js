...
Based on this recipe, generate relevant tags that would help categorize and find this recipe. Tags should be single words or short phrases with no punctuation, separated by commas. Focus on cuisine type, cooking method, dietary restrictions, difficulty level, and key ingredients (main protein). Return only the tags. In addition YOU WILL NOT add additional parenthesis like (potentially) or (implied), no explanation just the tags, the user will decide to use them or not.
        `;
